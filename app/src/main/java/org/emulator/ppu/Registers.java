...
public class Registers {
    ...
    public static void setVBlank(boolean vBlank) {
        byte status = Ram.read(CONTROL_ADDRESS);
        if (vBlank)
            status |= V_BLANK_MASK;
        else
            status &= ~V_BLANK_MASK;

        Ram.write(CONTROL_ADDRESS, status);
    }
    ...
    public static int getSize() {
        byte statusRegister = getControlRegister();
        boolean isSet = (statusRegister & 0x04) != 0;
        return isSet ? 16 : 8;
    }

    public static boolean isNMIMaskEnabled() {
        return (getControlRegister() & 0x80) != 0;
    }

    public static boolean getVblank() {
        return (getControlRegister() & V_BLANK_MASK) != 0;
    }
